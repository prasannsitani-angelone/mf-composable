...
<section class="p-4 px-1 pb-2 {$$props?.class}">
  <article class="flex items-center">
    <section class="flex-1 p-2">
      ...
      <div class="text-sm font-medium text-black-title uppercase">{autopay?.authenticationMode}</div>
    </section>

    <section class="flex-1 p-2">
      <div class="flex items-center text-[11px] font-normal text-grey-body">
        <div>Max. Autopay Limit</div>
        <WmsIcon name="question-mark-circle" class="ml-1" width={12} height={12} on:click={toggleShowMaxAutopayLimitModal} />
      </div>
      <div class="text-sm font-medium text-black-title uppercase"><AmountText amount={autopay?.amount} /></div>
    </section>
  </article>

  <article class="flex items-center mt-1">
    <section class="flex-1 p-2">
    ...
    <section class="flex-1 p-2">
      ...
      <div class="text-sm font-medium text-black-title">{getDateTimeString(new Date(autopay?.mandateStartDate)?.valueOf(), 'DATE', true)}</div>
    </section>
  </article>
</section>
...
{#if showMaxAutopayLimitModal}
  ...
  >
    ...
    <svelte:fragment slot="bodySection">
			<span />
      <section class="px-4 pt-0 pb-6 md:px-8 md:pt-6 text-sm font-normal text-grey-body">
        <p>
          This is just the Maximum amount that can be deducted from your bank account on a single day of you had an cumulative SIP of 1 lac for a day.
        </p>
        <p>
          Actual amount deducted will be your SIP amount. This limit is 1 lac to  cover for your future SIPs as well.
        </p>
      </section>
		</svelte:fragment>
  </InfoModal>
