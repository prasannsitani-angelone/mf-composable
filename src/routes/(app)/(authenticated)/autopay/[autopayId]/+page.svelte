...
<article class="mb-2" data-testid="autopayDetails">
    ...
    <section>
        ...
        <svelte:fragment slot="header">
          {#if response?.data?.umrnNo?.length}
            <div class="-mt-4 mb-2 text-[10px] font-medium text-grey-body uppercase">{response?.data?.authenticationMode}</div>
          {:else}
            <span />
          {/if}
        </svelte:fragment>
  ...
  {#await data.api.sipsLinked}
    ...
    <section>
      {#if response?.status === 'success' && response?.data}
        <article class="mt-2 p-3 text-black-title bg-white rounded-lg shadow-csm">
          ...
          <section class="mt-4">
              ...
              >
                <LinkedSipCard
                  ...
                  logoUrl={sip?.logoUrl}
                  class={`${index > 0 ? 'pt-4 border-t border-grey' : ''} ${index < response?.data?.sips?.length - 1 ? 'mb-3' : ''}`}
                />
              </div>
            {/each}
          </section>
