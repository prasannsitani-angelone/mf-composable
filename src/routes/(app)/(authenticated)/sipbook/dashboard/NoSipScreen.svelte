...
>
	<NoSipImage />
	<div class="mt-2 text-center text-lg font-normal text-black-title">
		You do not have any active SIP
	</div>
	...
	<Link to={`/explorefunds/high-returns?id=19`}>
		<div
			class="mt-5 w-52 cursor-default rounded border border-blue-primary bg-white py-4 text-center text-sm font-medium text-blue-primary active:opacity-95 md:cursor-pointer"
		>
			START SIP
		</div>
	</Link>
	...
	<Link to="/sipbook/inactivesips">
		{#if showInactiveSipsCta}
			<section class="mt-7 cursor-default text-center text-sm font-medium text-blue-primary">
				INACTIVE SIPs
			</section>
		{/if}
	</Link>
