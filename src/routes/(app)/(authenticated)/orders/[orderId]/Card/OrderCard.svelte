...
<div class="grid grid-cols-1 divide-y rounded-lg bg-background-alt px-3 py-4 shadow-csm">
	...
	<div class={`grid grid-cols-3 items-center justify-between  divide-x pt-3 ${$$props.class}`}>
		<div class="text-left">
			<span class="text-1xs text-body md:text-xs">Amount</span>
			<p class="overflow-hidden text-ellipsis whitespace-nowrap font-normal text-title">
		...
		<div class="text-center">
			<span class="text-1xs text-body md:text-xs">Order Type</span>
			<p class="overflow-hidden text-ellipsis whitespace-nowrap font-normal text-title">
		...
		<div class="text-right">
			{#if orderDetails?.status?.toUpperCase() === ORDER_STATUS.ORDER_COMPLETE}
				<span class="text-1xs text-body md:text-xs"> NAV date </span>
				<p class="overflow-hidden text-ellipsis whitespace-nowrap font-normal text-title">
			...
			{:else if orderDetails?.transactionType?.toUpperCase() === TRANSACTION_TYPE.REDEEM}
				<span class="flex items-center justify-end gap-1 text-1xs text-body md:text-xs">
					Ex. Credit Date
				</span>
				<p class="overflow-hidden text-ellipsis whitespace-nowrap font-normal text-title">
						...
						>
							<svelte:fragment slot="popupHeader">
								<div class="flex items-center justify-between px-4 pb-3 pt-6 md:px-8 md:py-6">
									<span class="text-lg font-normal text-title md:text-xl"> Expected NAV Date </span>
								</div>
							</svelte:fragment>
						</InfoPopup>
					{/if}
	...
	{#if showStatusNote}
		<div class="mt-3 rounded bg-tint12-primary px-3 py-2 text-sm text-title">
