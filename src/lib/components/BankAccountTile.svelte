<script setup lang="ts">
	...
	export let showRadioButton = false;
	export let identifier = 0;
	export let selected = false;
	export let clazz = '';

	export let onSelect: (payload: number) => void = () => undefined;
</script>

<!-- svelte-ignore a11y-click-events-have-key-events -->
<div
	class={`flex w-full flex-col justify-between rounded border bg-background-alt px-4 py-3 ${clazz}`}
	on:click={() => onSelect(identifier)}
>
	<div class="flex items-center justify-between">
		<div class="flex flex-row items-center">
			<div class="flex h-8 w-11 items-center justify-center rounded-sm border">
