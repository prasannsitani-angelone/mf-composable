...
<section
	class="flex items-start justify-between rounded-lg border bg-background-alt p-3 md:cursor-pointer {$$props?.class ||
		''}"
	on:click={onOptionClick}
>
	<section class="flex items-center">
		<article
			class="flex h-9 w-9 items-center justify-center rounded-full bg-tint12-primary p-2 text-sm font-normal text-title"
		>
			<div class="flex">
				{memberNameInitials}
			</div>
		</article>
		<article class="ml-2 font-normal text-title">
			<article class="flex items-center">
				...
				{#if familyMember?.relation?.toLowerCase() !== 'self'}
					<div class="ml-1 rounded-sm bg-background px-1 py-0.5 text-[10px] uppercase">
						{familyMember?.relation}
					</div>
				{/if}
			</article>
			<div class="text-xs capitalize text-body">
