...
<Modal isModalOpen={true} on:backdropclicked={onBackDropClicked}>
	<div
		class="flex w-full flex-col items-center justify-between overflow-y-auto rounded-t-2xl bg-background-alt pt-4 shadow-clg sm:w-120 sm:rounded-lg sm:px-20 sm:pb-12 sm:pt-16 ${$$props.class}"
	>
		<slot name="header">
		...
		<slot name="content">
			<div class="w-full text-center">
				<div class="mb-4 px-4 text-2xl font-normal text-title sm:mb-2">
					{heading}
				</div>
				<div class="flex flex-col gap-1 px-4 text-sm text-body">
		...
		<slot name="footer" class="w-full">
			<div class="flex w-full flex-col items-center px-4 pt-5">
				{#if link}
					<a href={link} target="_blank" class="my-3 text-sm font-bold text-primary">KNOW MORE</a>
				{/if}
